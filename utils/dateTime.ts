export function getDateFormalString(date: Date) {
    ...
    else if (date > lastWeek) return date.toLocaleDateString('en-US', { weekday: 'long' });
    else if (date > lastMonth) return `${Math.floor((today.getTime() - date.getTime()) / (1000 * 60 * 60 * 24 * 7))} weeks ago`;
    else if (date > lastYear) return `${Math.floor((today.getTime() - date.getTime()) / (1000 * 60 * 60 * 24 * 30))} months ago`;
    else if (date > lastDecade) return `${Math.floor((today.getTime() - date.getTime()) / (1000 * 60 * 60 * 24 * 365))} years ago`;
