<template>
    <div class="mt-2 sm:mx-auto sm:w-full sm:max-w-[580px] px-6 sm:px-12">
        <h2 class="text-left text-4xl font-bold leading-9 tracking-tight text-gray-900">Welcome back! 👋</h2>
            ...
            <div>
                <form class="space-y-6" method="POST" @submit.prevent="onLogin">
                    <span class="p-input-icon-left w-full">
                        <i class="pi pi-user" />
                        <InputText v-model="email" type="text" placeholder="Username" class="w-full" :class="{ 'p-invalid': !isEmailValid }" :onChange="onDataChange" />
                        <small v-if="!isEmailValid && !!email" class="p-error" id="mask-error">{{ 'This email is invalid' || '&nbsp;' }}</small>
                    </span>
                    <div>
                        <span class="p-input-icon-left w-full">
                            <i class="pi pi-eye" />
                        ...
                        <small v-if="!isPasswordValid && !!password" class="p-error" id="mask-error">{{ 'Password must be at least 6 characters' || '&nbsp;' }}</small>
                    </div>
